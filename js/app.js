import { stations, services } from "./data.js?v=drift-01";
import { directions, modes, featureLabels, timeBands, plan } from "./planner.js?v=drift-01";

const state = { origin: "kanazawa", mode: "normal", direction: "any", latestMinutes: 420, transfers: 2, features: [], timeBand: "flexible", excluded: [] };
...
function directionButtons() {
  return Object.entries(directions).map(([key, direction]) => '<button class="time-band ' + (state.direction === key ? "selected" : "") + '" data-direction="' + key + '">' + direction.icon + " " + direction.label + "</button>").join("");
}
...
function timeBandButtons() {
  return Object.entries(timeBands).map(([key, band]) => '<button class="time-band ' + (state.timeBand === key ? "selected" : "") + '" data-time-band="' + key + '">' + band.label + "</button>").join("");
}
function card(option) {
  ...
  const detail = option.stay ? "No rail leg required. Keep the day loose." : option.edges.map((edge) => edge.line + " → " + stationMap.get(edge.to).name).join("<br>");
  ...
  const journeyLabel = option.stay ? "same place · no rail work" : route;
  const actions = option.stay ? '<button class="day-sheet-button" data-plan="' + option.destination.id + '">Keep this day <span>↗</span></button>' : '<div class="card-actions"><button class="day-sheet-button" data-plan="' + option.destination.id + '">Open the day <span>↗</span></button><button class="secondary-action" data-start-here="' + option.destination.id + '">Start from here</button><button class="not-today" data-not-today="' + option.destination.id + '" aria-label="Not ' + option.destination.name + ' today">×</button></div>';
  return '<article class="card ' + (option.stay ? "stay-card" : "") + '"><div class="card-top"><span class="kind">' + option.kind + '</span><span class="confidence ' + option.stats.confidence + '">' + (option.stay ? "zero commitment" : option.stats.confidence + " confidence") + '</span></div><div class="card-place"><span class="place-dot"></span><h2>' + option.destination.name + '</h2><span class="arrow">' + (option.stay ? "•" : "↘") + '</span></div><p class="journey">' + journeyLabel + '</p><div class="duration"><strong>' + (option.stay ? "TODAY" : Math.floor(duration / 60) + '<small>h</small> ' + duration % 60 + '<small>m</small>') + '</strong><span>' + (option.stay ? "keep the hotel<br>decide later" : "typical journey<br>including wait") + '</span></div><ul>' + reasonList + '</ul><div class="traits">' + traits + '</div><div class="activities"><p>WHEN YOU GET THERE</p><ol>' + activities + '</ol></div>' + actions + '<details><summary>' + (option.stay ? "What makes today easy" : "Route at a glance") + ' <span>+</span></summary><p>' + detail + "</p></details></article>";
}
...
function render() {
  ...
  const results = plan({ stations, services, origin: state.origin, latestMinutes: effectiveMinutes, maxTransfers: state.mode === "cooked" ? 0 : state.transfers, mode: state.mode, direction: state.direction, desiredFeatures: state.features, timeBand: state.timeBand, includeOrigin: originMatches || ["quiet", "cooked"].includes(state.mode), excluded: state.excluded });
  ...
  const now = new Date();
  app.innerHTML = '<main><header class="masthead"><a class="wordmark" href="#" aria-label="Go Forth home">GO <span>FORTH</span></a><p>JAPAN RAILWAY POSSIBILITY ENGINE <i></i> v0.1</p><button class="about" data-about aria-label="About this prototype">?</button></header><section class="today-bar"><span>TODAY</span><b>' + stationMap.get(state.origin).name + '</b><time>' + time(now.getHours() * 60 + now.getMinutes()) + '</time><small>arrive by ~' + time(540 + effectiveMinutes) + '</small></section><section class="hero"><div class="hero-copy"><p class="eyebrow">STARTING FROM</p><div class="location-row"><span class="pin">✦</span><select id="origin" aria-label="Starting station">' + originOptions + '</select></div><h1>Where could today <em>take you?</em></h1><p class="lede">One decision at a time. Moving, staying, and changing your mind all count.</p></div><div class="map-wrap"><div class="map-glow"></div><svg class="network" viewBox="0 0 100 100" role="img" aria-label="Illustrative network diagram">' + mapLines() + mapStations() + '</svg><p class="map-key"><span></span> hand-drawn starter network</p></div></section><section class="controls" aria-label="Adventure preferences"><div class="mode-scroller">' + modeButtons() + '</div><div class="limits"><label><span>Home by</span><output id="time-output">' + time(540 + state.latestMinutes) + '</output><input id="time" type="range" min="360" max="780" step="30" value="' + (540 + state.latestMinutes) + '" aria-label="Latest arrival time"></label><label><span>Changes</span><output id="transfer-output">' + (state.mode === "cooked" ? "Direct only" : state.transfers === 0 ? "Direct only" : state.transfers + " or fewer") + '</output><input id="transfers" type="range" min="0" max="3" step="1" value="' + state.transfers + '" aria-label="Maximum changes"' + (state.mode === "cooked" ? " disabled" : "") + '></label></div></section><section class="results" aria-live="polite"><div class="section-head"><div><p class="eyebrow">YOUR POSSIBILITIES</p><p class="muted">' + modes[state.mode].label + " · " + directions[state.direction].label + " · " + timeBands[state.timeBand].label + " · leave now" + '</p></div><button class="reroll" data-reroll>None of these · show more ↻</button></div><div class="cards">' + (results.length ? results.map(card).join("") : '<div class="empty"><b>Nothing feels comfortably possible yet.</b><p>Try adding time, allowing one more change, or clearing today’s skips.</p></div>') + '</div></section><aside class="notice" id="notice"><b>NOT A TIMETABLE.</b> This is an illustrative planning prototype using approximate, hand-authored service patterns. Check current official railway information before boarding.</aside></main>';
  document.querySelector(".mode-scroller").insertAdjacentHTML("afterend", '<div class="feature-area"><p>WHICH WAY SHOULD TOMORROW POINT?</p><div class="time-strip">' + directionButtons() + '</div><p>WHAT SOUNDS GOOD?</p><div class="feature-strip">' + featureButtons() + '</div><p class="time-question">HOW MUCH RAIL?</p><div class="time-strip">' + timeBandButtons() + "</div></div>");
  app.insertAdjacentHTML("beforeend", '<dialog id="day-plan"></dialog>');
  bind();
}
function bind() {
  document.querySelector("#origin").addEventListener("change", (event) => { state.origin = event.target.value; state.excluded = []; render(); });
  document.querySelectorAll("[data-mode]").forEach((button) => button.addEventListener("click", () => { state.mode = button.dataset.mode; render(); }));
  document.querySelectorAll("[data-direction]").forEach((button) => button.addEventListener("click", () => { state.direction = button.dataset.direction; render(); }));
  ...
  document.querySelectorAll("[data-time-band]").forEach((button) => button.addEventListener("click", () => { state.timeBand = button.dataset.timeBand; render(); }));
  ...
  document.querySelectorAll("[data-start-here]").forEach((button) => button.addEventListener("click", () => { state.origin = button.dataset.startHere; state.excluded = []; render(); }));
  document.querySelectorAll("[data-not-today]").forEach((button) => button.addEventListener("click", () => { state.excluded = [...state.excluded, button.dataset.notToday]; render(); }));
  document.querySelector("[data-reroll]").addEventListener("click", () => { state.excluded = [...state.excluded, ...visibleResults.filter((result) => !result.stay).map((result) => result.destination.id)]; render(); });
